<?php
	...
	class Putaway extends MY_Controller {
			...
			public function index(){
				 ...
				 $data = array();
				 $this->includeJS[]='assets/js/page/putaway.js';
				 $this->template->set("title", "<i class=\"fas fa-file\"></i> Putaway");
				 $this->template->set("titlepage", "Putaway");
				 $this->template->newload("pages/Putaway", array() , $data);
				 
			}

			public function findItemkey(){
				...
				if($param['itemkey'] != ''){
					$lotdetails = $bme->select('
												...
												')->where('INMAST.ItemKey',$this->db->escape($param['itemkey']))
												  ->where('DateExpiry >=GETDATE()')
												  ->where('QtyOnHand > 0')
												->join('INMAST','INMAST.Itemkey = LotMaster.ItemKey','inner')
												->group_by('INMAST.ItemKey,LocationKey,Desc1,Stockuomcode')
												->get('LotMaster')
												->result();
					$msg['data'] = $lotdetails;	
				}
			...
			public function sendputaway(){
				...
				if($lotdetails->QtyOnHand == $putaway){
					...
					if($lotexist){
						...
						$this->models->Update("LotMaster",$uLotMaster,["LotNo" => $bme->escape($lotno),"BinNo" => $bme->escape($tobin),"Itemkey" => $bme->escape($itemkey)]);
