...
APP.Datatable.defaultOpts.url="/bme/RM_Label49/dataTable";
...
function getdatainfo(lotno = '',itemkey =''){
    ...
    $.post('RM_Label49/findtransno',{itemkey:itemkey, lotno:lotno},function(res){
       var recx = res.data;
        if(res.msgno == "100" ){
...
function getsearchdata(mm = ''){
    ...
    $.post('RM_Label49/findreceiptdoc',{ReceiptDocNo:mm},function(res){
        var recx = res.data;
         if(res.msgno == "100" ){
        ...
        bootbox.prompt({ 
            ...
            callback: function (result) {
                if(result){
                    bootbox.dialog({
                        size:'xl',
                        message:"<iframe id='xx' src='"+ 'RM_Label49/print?count='+result+'&lotno='+$('#lotno').val() +"&ItemKey="+$('#ItemKey').val() +"' style='width:100%;min-height:75vh'></iframe>",
                      })
                }
            }
        });
