...
@Component({
  selector: 'app-inventory-validation',
  templateUrl: './inventory-validation.page.html',
  styleUrls: ['./inventory-validation.page.scss'],
})
export class InventoryValidationPage implements OnInit {
  ...
  async tfind(keyCode:number){
    ...
    if(keyCode == 13 && this.search_barcode != '' && this.search_bin != ''){
      ...
      (await this.api.post({ "binno": this.search_bin , 'barcode' : this.search_barcode }, 'InventoryCount/findVerifyCount', "")).subscribe(
        async (res) => {
          if(res.msg != ""){
  ...
  async btnSave(){
    if(this.search_barcode != '' && this.search_bin != '' && (this.qty != '' || parseFloat(this.qty) > 0 )){
      ...
      (await this.api.post({ "binno": this.search_bin , 'barcode' : this.search_barcode, 'qty' : this.qty , 'invid' : this.invid}, 'InventoryCount/saveVerifyCount', "")).subscribe(
        async (res) => {
          if(res.msgno != "200"){
