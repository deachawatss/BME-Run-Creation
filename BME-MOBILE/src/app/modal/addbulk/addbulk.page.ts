...
export class AddbulkPage implements OnInit {
  ...
  async deleteme(e:any){
    var dt = await this.pickedData[e];
    console.log(dt);
    //return false;
    await (await this.api.post({dataid: dt.tbl_rm_allocate_bulk_id, itemkey: dt.itemkey, runno: dt.runno , batchno: dt.batchno}, 'Bulkpick/removeitem', "")).subscribe(
      async (res) => {
          if(res != null){

            
            if(res.msgno == '200'){ 
              this.pickedData = await res.data;
              this.presentToast(res.msg);
            }else{
              this.presentToast(res.msg);
            }
